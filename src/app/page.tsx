...
export default function Home() {
  ...
  // Busca a coleção de pets otimizada com limite para reduzir custos
  ...
  return (
    <div className="flex flex-col min-h-screen bg-black text-white selection:bg-primary selection:text-black">
      ...
      <main className="flex-1 container mx-auto px-4 py-8 md:py-24">
        ...
        <section className="max-w-4xl mx-auto mb-20 md:mb-32">
          {pets && pets.length > 0 ? (
            ...
            <Link href="/cadastrar-pet">
              <div className="group relative bg-white/5 border border-white/10 rounded-[1.5rem] md:rounded-[2rem] p-6 md:p-10 hover:bg-white/10 transition-all duration-500 hover:border-primary/40 cursor-pointer overflow-hidden shadow-2xl">
                ...
                <div className="flex items-center gap-3 mb-4 md:mb-6">
                  <div className="bg-primary/20 p-2 md:p-3 rounded-xl group-hover:scale-110 transition-transform duration-500">
                    <PlusCircle className="w-6 h-6 md:w-8 md:h-8 text-primary" />
                  </div>
                  <h2 className="text-xl md:text-3xl font-headline font-bold text-white tracking-tight">
        ...
        <section className="mt-12 md:mt-20 py-8 md:py-12 border-y border-white/5 text-center">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-10 max-w-5xl mx-auto px-4">
            <div className="space-y-2 md:space-y-3">
              <div className="w-8 h-8 md:w-10 md:h-10 bg-primary/10 rounded-full flex items-center justify-center mx-auto mb-2 md:mb-4">
                <Zap className="w-4 h-4 md:w-5 md:h-5 text-primary" />
              </div>
              <h3 className="text-base md:text-lg font-bold tracking-tight">Análise Neural</h3>
              <p className="text-[10px] md:text-xs text-muted-foreground leading-relaxed">Processamento avançado para insights instantâneos sobre nutrição.</p>
            </div>
            <div className="space-y-2 md:space-y-3">
              <div className="w-8 h-8 md:w-10 md:h-10 bg-primary/10 rounded-full flex items-center justify-center mx-auto mb-2 md:mb-4">
                <ShieldCheck className="w-4 h-4 md:w-5 md:h-5 text-primary" />
              </div>
              <h3 className="text-base md:text-lg font-bold tracking-tight">Privacidade Total</h3>
              <p className="text-[10px] md:text-xs text-muted-foreground leading-relaxed">Seus dados e fotos protegidos por criptografia de ponta a ponta.</p>
            </div>
            <div className="space-y-2 md:space-y-3">
              <div className="w-8 h-8 md:w-10 md:h-10 bg-primary/10 rounded-full flex items-center justify-center mx-auto mb-2 md:mb-4">
                <HeartPulse className="w-4 h-4 md:w-5 md:h-5 text-primary" />
              </div>
              <h3 className="text-base md:text-lg font-bold tracking-tight">Cuidado Preditivo</h3>
              <p className="text-[10px] md:text-xs text-muted-foreground leading-relaxed">Identificamos padrões para prevenir problemas futuros de saúde.</p>
            </div>
          </div>
        </section>
      </main>
      ...
      <footer className="bg-black py-6 md:py-8 border-t border-white/5">
        <div className="container mx-auto px-4 text-center">
          <p className="text-[7px] md:text-[8px] text-muted-foreground uppercase tracking-[0.3em] opacity-50 font-bold mb-2 md:mb-3">Vet AI Elite Technology WS Studios</p>
          <p className="text-[7px] md:text-[8px] opacity-40 uppercase tracking-widest leading-relaxed">
            AVISO: FERRAMENTA DE AUXÍLIO TECNOLÓGICO. <br className="md:hidden" />
